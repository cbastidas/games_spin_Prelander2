...
alignToWinner(winnerIndex: number) {
  const cardWidth = 170; // 160 de la imagen + 10 de margen
  const targetPosition = 3; // siempre la 4ª carta (índice 3) queda bajo la línea verde

  // diferencia de posiciones
  const offset = winnerIndex - targetPosition;

  // cuánto debemos mover en px
  const targetTranslate = -(offset * this.cardWidth) - (this.cardWidth / 2);

  // cuando la velocidad llegue casi a 0, forzamos la alineación exacta
  const checkInterval = setInterval(() => {
    if (this.scrollSpeed <= 0.5) {
      this.trackTranslateX = targetTranslate;
      this.scrollSpeed = 0;
      this.isSpinning = false;
      this.stopping = false;
